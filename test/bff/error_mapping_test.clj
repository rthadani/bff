(ns bff.error-mapping-test
  "Tests for PR 7: spec-level errors mapping + resolver-returned :code
   surfaced via extensions.code."
...
(deftest test-step-errors-mapping-by-http-status
  (let [step (assoc base-step :errors {400 "DUPLICATE_CUSTOMER"})]
    (with-redefs [http/call (fn [_] (http/err :bad-request "400" {} 400))]
      (let [{:keys [errors]} (run-sync! (executor/run-endpoint
                                          (assoc base-endpoint :backend_chain [step])
                                          {} {}))]
